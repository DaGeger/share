...
function delFile(mpdobj, key, callback){
...
function addFile(mpdobj, flist, callback){
	if (!flist) {return;}
...
MUpload.prototype.init = function(){
	...
	//删除文件
	$(options.container).on('click', '.del-item', that, function(e){
		var that = e.data, key = $(this).data('itemid');
		delFile(that, key, check_data);
	});

	//添加文件
	$(options.container).on('change','.layui-upload-file', that, function(e){
		if(!this.files || this.files.length == 0){
			return;
		}
		that = e.data;
		addFile(that, this.files,check_data);
	});
