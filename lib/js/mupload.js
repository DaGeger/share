...
function delFile(mpdobj, key){
	...
	$('#'+key).remove();
	var len = Object.keys(mpdobj.fileobj).length;
	change_del_btn(mpdobj.options.container+' .layui-mupload-submit', len);
	changeNoFileBox(mpdobj.options.container, len);
}
 ...
 */
MUpload.prototype.handlerDrag = function(){

	var that = this,
		container = that.options.container,
		elemObj = $(container + ' tbody')[0];
    elemObj.addEventListener('dragover', function(ev){ev.preventDefault();}, false);
